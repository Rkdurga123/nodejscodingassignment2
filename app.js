...
const authenticateToken = (request, response, next) => {
  ...
  if (jwtToken === undefined) {
    ...
    jwt.verify(jwtToken, 'Rkdurga_key', async (error, payload) => {
      if (error) {
        response.status(401)
        response.send('Invalid JWT Token')
      } else {
...
app.post('/login/', async (request, response) => {
  ...
  const jwtToken = jwt.sign(payload, 'Rkdurga_key')
  const userCheckQuery = `SELECT * FROM user WHERE username='${username}';`
  const dbUser = await db.run(userCheckQuery)
  if (dbUser === undefined) {
    response.status(400)
...
const isUSerFollowing = async (request, response, next) => {
  ...
  let isTweetUserIdFollowingIds = false
  userFollowingData.forEach(Each => {
    if (each['following_user_id'] === tweetUserId) {
      isTweetUserIdFollowingIds = true
    }
  })
  if (isTweetUserIdFollowingIds) {
    next()
  } else {
    response.status(400)
...
app.get('/user/tweets/feed/', authenticateToken, async (request, response) => {
  ...
  const query = `SELECT username,tweet,date_time As dateTime FROM follower INNER JOIN tweet ON follower.following_user_id=tweet.user_id
    NATURAL JOIN user WHERE follower.following_user_id=${userId}
...
app.get('/tweets/:tweetId/', authenticateToken, async (request, response) => {
  const {tweetId} = request.params
  const query = `SELECT tweet, COUNT() AS replies, date_time AS dateTime FROM tweet INNER JOIN reply
...
app.get(
  ...
  async (request, response) => {
    ...
    const data = await db.all(query)
    const usernamesArr = data.map(each => each.username)
    response.send({likes: usernamesArr})
  },
)

app.get(
  '/tweets/:tweetIDd/replies/',
  authenticateToken,
  async (request, response) => {
    const {tweetId} = request.params
    const query = `SELECT name,reply FROM user WHERE tweet_id=${tweetId};`
    const data = await db.all(query)
...
app.get('/user/tweets/', authenticateToken, async (request, response) => {
  ...
  const query = `SELECT tweet, COUNT() AS likes, date_time AS dateTime FROM tweet INNERJOIN like
    ON tweet.tweet_id=like.tweet_id
    WHERE tweet.user_id=${userId}
    GROUP BY tweet.tweet_id;`
  let likesData = await db.all(query)
  const repliesQuery = `SELECT tweet COUNT() AS replies FROM tweet INNER JOIN reply
    ON tweet.tweet_id=reply.tweet_id
    WHERE tweet.user_id=${userId}
    GROUP BY tweet.tweet_id;`
...
app.delete(
  ...
  async (request, response) => {
    ...
    const userTweetsQuery = `SELECT tweet_id,user_id FROM tweet 
